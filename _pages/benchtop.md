...
Even though the system uses a rolling release model, it has LTS core packages and a GNOME old stable desktop. Security and bug fixes arrive promptly as point releases, and GNOME moves to its next major version only when upstream ends support for the current one. Every update is a snapshot that can be rolled back.
...
It runs on x86 and ARM workstations. The LTS and Current kernels include patches for laptops, tablets, convertibles, and common peripherals.
